...
export const createPin = asyncHandler(async (req, res) => {
    ...
    const localFilePath = req.file?.path; // Assuming multer handles the upload

    if (!localFilePath) {
        throw new ApiError(400, "Image field is necessary to create pins.");
    }

    const image = await uploadImageToCloudinary(localFilePath);
...
export const getAllPins = asyncHandler(async (req, res) => {
    ...
    if (!pins || pins.length === 0) {
        throw new ApiError(404, "No Pins exists.")
    }
    return res
        .status(200)
        .json(
